...
Runs on the main thread for the lifetime of the service and, on every pass,
reconciles what the database believes about each analysis with what Kubernetes
and the analysis containers actually report. It polls each analysis' internal
status endpoint, promotes statuses forward, restarts stuck deployments,
streams logs, pushes status updates to the FLAME Hub, and archives and cleans
up analyses that have finished.
"""
...
def status_loop(
    database: Database, status_loop_interval: int, namespace: str = "default"
) -> None:
    ...
    while True:
        if hub_client is None:
            ...
            if None not in client_params:
                ...
                logger.error(
                    f"One or more hub client initialization parameters are None.\n"
                ...
                raise ValueError(
                    "One or more hub client initialization parameters are None."
                )
                ...
                for analysis_id in running_analyzes:
                    ...
                    if analysis_id not in node_analysis_ids.keys():
                        node_analysis_id = get_node_analysis_id(
                            hub_client, analysis_id, node_id
                        )
                        if isinstance(node_analysis_id, str):
                            node_analysis_ids[analysis_id] = node_analysis_id
                        else:
                            logger.warning(
                                f"Retrieving node_analysis id for malformed analysis returned None "
                    ...
                    if node_analysis_id is not None:
                        try:
                            ...
                            _ = inform_analysis_of_partner_statuses(
                                database, hub_client, analysis_id, node_analysis_id
                            )
                        except Exception as e:
                            logger.status_loop(
                                f"Error when attempting to access partner_status endpoint of "
                        ...
                        if analysis_status["status_action"] == "unstuck":
                            ...
                            analysis_status = _get_analysis_status(
                                analysis_id, database
                            )
                            if analysis_status is None:
                                continue
                        ...
                        analysis_hub_status = _set_analysis_hub_status(
                            hub_client, node_analysis_id, analysis_status
                        )
                        logger.info(
                            f"Set Hub analysis status with node_analysis={node_analysis_id}, "
            ...
            logger.status_loop(
                f"Iteration completed. Sleeping for {status_loop_interval} seconds."
            )
            time.sleep(status_loop_interval)


def inform_analysis_of_partner_statuses(
    database: Database,
    ...
    try:  # try except, in case analysis api is not yet ready
        ...
        logger.warning(
            f"Error whilst trying to access analysis partner_status endpoint: {repr(e)}"
        )
    except ConnectError as e:
        logger.warning(
            f"Connection to http://nginx-{deployment_name}:{PORTS['service'][0]} yielded an error: {repr(e)}"
        )
    except (TimeoutException, ConnectTimeout) as e:
        logger.warning(
            f"Connection to http://nginx-{deployment_name}:{PORTS['service'][0]} timed out: {repr(e)}"
        )
    client.close()
    return None
...
def _get_analysis_status(
    analysis_id: str, database: Database
) -> Optional[dict[str, str]]:
    ...
    if analysis is not None:
        ...
        if db_status in [
            ...
            int_status = _get_internal_deployment_status(
                analysis.deployment_name, analysis_id
            )
        status_action = _decide_status_action(analysis.status, int_status)
        logger.status_loop(
            f"Calculating action: db_status={analysis.status}, int_status={int_status} -> status_action={status_action}"
        )
        return {
...
def _decide_status_action(db_status: str, int_status: str) -> Optional[str]:
    ...
    """
    is_stuck = (db_status not in [AnalysisStatus.FAILED.value]) and (
        int_status in [AnalysisStatus.STUCK.value]
    )
    is_slow = (db_status in [AnalysisStatus.STARTED.value]) and (
        int_status in [AnalysisStatus.FAILED.value]
    )
    newly_running = (db_status in [AnalysisStatus.STARTED.value]) and (
        int_status in [AnalysisStatus.EXECUTING.value]
    )
    speedy_finished = (db_status in [AnalysisStatus.STARTED.value]) and (
        int_status in [AnalysisStatus.EXECUTED.value]
    )
    newly_ended = (
        db_status in [AnalysisStatus.EXECUTING.value, AnalysisStatus.FAILED.value]
    ) and (int_status in [AnalysisStatus.EXECUTED.value, AnalysisStatus.FAILED.value])
    firmly_stuck = (db_status in [AnalysisStatus.FAILED.value]) and (
        int_status in [AnalysisStatus.STUCK.value]
    )
...
def _get_internal_deployment_status(deployment_name: str, analysis_id: str) -> str:
    ...
    while True:
        try:
            response = client.get(
                "/analysis/healthz", headers=[("Connection", "close")]
            )
            response.raise_for_status()
            client.close()
            break
            ...
            logger.warning(
                f"Error whilst retrieving internal deployment status: {repr(e)}"
            )
        except ConnectError as e:
            logger.warning(
                f"Connection to http://nginx-{deployment_name}:{PORTS['service'][0]} yielded an error: {repr(e)}"
        ...
        if elapsed_time > _INTERNAL_STATUS_TIMEOUT:
            logger.error(
                f"Timeout getting internal deployment status after {elapsed_time:.1f} seconds"
            )
            client.close()
    ...
    analysis_status, analysis_token_remaining_time = (
...
def _fix_stuck_status(
    ...
    if analysis is not None:
        ...
        if analysis.restart_counter < _MAX_RESTARTS:
            _stream_stuck_logs(
                analysis, node_id, enable_hub_logging, database, hub_client, is_slow
            )
            unstuck_analysis_deployments(analysis_status["analysis_id"], database)
        else:
            database.update_deployment_status(
                analysis.deployment_name, status=AnalysisStatus.FAILED.value
            )
            _stream_stuck_logs(
                analysis, node_id, enable_hub_logging, database, hub_client, is_slow
            )
    ...
    if is_slow:
        ...
        if pod_status_dict is not None:
            ...
            ready, reason, message = (
                pod_status_dict["ready"],
                pod_status_dict["reason"],
                pod_status_dict["message"],
            )
            # ready=True implicates slow state, else assume kubernetes_error state
            if not ready:
                is_k8s_related = True
                logger.error(
                    f"Deployment of analysis={analysis.analysis_id} failed (ready={ready}). "
                    f"{reason}: {message}"
                )
...
def _update_running_status(database: Database, analysis_status: dict[str, str]) -> None:
    ...
    if analysis is not None:
        database.update_deployment_status(
            analysis.deployment_name, AnalysisStatus.EXECUTING.value
        )


def _update_finished_status(
    database: Database, analysis_status: dict[str, str]
) -> None:
    ...
    if analysis is not None:
        ...
        if analysis_status["int_status"] == AnalysisStatus.EXECUTED.value:
            ...
            delete_analysis(
                analysis_status["analysis_id"], database
...
def _set_analysis_hub_status(
    ...
    """
    if analysis_status["db_status"] in [
    ...
    elif analysis_status["int_status"] in [
        AnalysisStatus.FAILED.value,
        AnalysisStatus.EXECUTED.value,
        AnalysisStatus.STOPPED.value,
